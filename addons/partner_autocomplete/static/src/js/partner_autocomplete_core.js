harpiya.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousHarpiya: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        var harpiyaSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var harpiyaPromise = self._getHarpiyaSuggestions(value, isVAT).then(function (suggestions){
                harpiyaSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with Harpiya result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = harpiyaSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            harpiyaSuggestions.push(suggestion);
                        }
                    });
                }

                harpiyaSuggestions = _.filter(harpiyaSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(harpiyaSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(harpiyaSuggestions);
            };

            self._whenAll([harpiyaPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    _getHarpiyaSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousHarpiya.add(def);
    },
    /**
