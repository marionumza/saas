...
from harpiya.addons.payment.tests.common import PaymentAcquirerCommon
from harpiya.addons.payment_authorize.controllers.main import AuthorizeController
...
@harpiya.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('harpiya.addons.payment_authorize.models.payment', 'ValidationError')
...
@harpiya.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on harpiya/harpiya + create sandbox account
        authorize.write({
