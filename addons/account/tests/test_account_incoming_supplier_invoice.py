...
class TestAccountIncomingSupplierInvoice(AccountingTestCase):
    ...
    def setUp(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('mail.catchall.domain', 'test-company.harpiya.com')

        self.internal_user = self.env['res.users'].create({
            'name': 'Internal User',
            'login': 'internal.user@test.harpiya.com',
            'email': 'internal.user@test.harpiya.com',
        })
