...
from harpiya.api import call_kw, Environment
from harpiya.modules import get_module_path, get_resource_path
from harpiya.tools import image_process, topological_sort, html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property
from harpiya.tools.mimetypes import guess_mimetype
from harpiya.tools.translate import _
from harpiya.tools.misc import str2bool, xlsxwriter, file_open
from harpiya.tools.safe_eval import safe_eval
from harpiya import http, tools
from harpiya.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception, Response
from harpiya.exceptions import AccessError, UserError, AccessDenied
from harpiya.models import check_method_name
from harpiya.service import db, security
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('harpiya.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, harpiya.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = harpiya.conf.server_wide_modules or []
    serverside = ['base', 'web']
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("harpiya.addons.web.main.manifest_list(): debug parameter is deprecated")
    mods = mods.split(',')
...
def fix_view_modes(action):
    """ For historical reasons, Harpiya has weird dealings in relation to
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=harpiya.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.harpiya.com/oauth2/auth?' + werkzeug.url_encode(params)
    ...
    def _content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if not image_base64:
            ...
            if not (width or height):
                width, height = harpiya.tools.image_guess_size_from_field_name(field)
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = harpiya.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: harpiya.tools.ustr(field[1].get('string', '').lower()))
