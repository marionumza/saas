...
(function () {
    ...
    harpiya.testing = typeof QUnit === 'object';
    harpiya.remainingJobs = jobs;
    harpiya.__DEBUG__ = {
    ...
    harpiya.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__harpiya_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: harpiya.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = harpiya.__DEBUG__.getDependencies(job.name);
            ...
            missing = harpiya.__DEBUG__.getMissingJobs();
            failed = harpiya.__DEBUG__.getFailedJobs();
            ...
            if (harpiya.debug || failed.length || unloaded.length) {
                ...
                if (harpiya.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        harpiya.__DEBUG__.jsModules = {
            missing: missing,
    ...
    harpiya.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            harpiya.processJobs(jobs, services);
