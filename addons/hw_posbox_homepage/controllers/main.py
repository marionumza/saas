...
from harpiya.addons.hw_proxy.controllers import main as hw_proxy
from harpiya.addons.web.controllers import main as web
from harpiya.modules.module import get_resource_path
from harpiya.addons.hw_drivers.tools import helpers
from harpiya.addons.hw_drivers.controllers.driver import iot_devices
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('harpiya.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/harpiya/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])

    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('harpiya-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_harpiya_server_url() or 'Not Configured',
    ...
    def list_drivers(self):
        ...
        return driver_list_template.render({
            'title': "Harpiya's IoT Box - Drivers list",
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "Harpiya's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('harpiya-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('harpiya-enterprise-code.conf'),
        })
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_harpiya_server_url() or '',
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> Harpiya server configuration',
            'breadcrumb': 'Configure Harpiya Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_harpiya_server_url() or 'Not configured yet',
    ...
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/harpiya/", "--git-dir=/home/pi/harpiya/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
        ...
        return upgrade_page_template.render({
            'title': "Harpiya's IoTBox - Software Upgrade",
    ...
    def perform_upgrade(self):
        ...
        os.system('/home/pi/harpiya/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/harpiya/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspbian(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/harpiya/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspbian']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspbian(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/harpiya/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspbian']).decode().split('\n')[-2]
