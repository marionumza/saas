...
from harpiya import _, api, exceptions, fields, models, tools, registry, SUPERUSER_ID
from harpiya.osv import expression

from harpiya.tools import pycompat, ustr
from harpiya.tools.misc import clean_context, split_every
   ...
   Only allowed people can contact us at this address.</p>
    ...
    def message_post(self, *,
        ...
        if values['author_id'] and values['message_type'] != 'notification' and not self._context.get('mail_create_nosubscribe'):
            # if self.env['res.partner'].browse(values['author_id']).active:  # we dont want to add harpiyabot/inactive as a follower
