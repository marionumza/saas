...
from harpiya.addons.sms.models.sms_sms import SmsSms as SmsSms
...
class LinkTrackerMock(common.BaseCase):
    ...
    def setUp(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('web.base.url', 'https://test.harpiya.com')

        link_tracker_favicon_patch = patch('harpiya.addons.link_tracker.models.link_tracker.LinkTracker._compute_favicon', wraps=_compute_favicon)
...
class TestSMSPost(test_mail_full_common.BaseFunctionalTest, sms_common.MockSMS, LinkTrackerMock):
    ...
    def test_body_link_shorten(self):
        ...
        link = 'https://test.harpiya.com/my/super_page?test[0]=42&toto=áâà#title3'
        self.env['link.tracker'].search([('url', '=', link)]).unlink()
        new_body = self.env['link.tracker']._convert_links_text('Welcome to %s !' % link, self.tracker_values)
        self.assertNotIn(link, new_body)
    ...
    def test_body_link_shorten_wshort(self):
        link = 'https://test.harpiya.com/r/RAOUL'
        self.env['link.tracker'].search([('url', '=', link)]).unlink()
        new_body = self.env['link.tracker']._convert_links_text('Welcome to %s !' % link, self.tracker_values)
    ...
    def test_body_link_shorten_wunsubscribe(self):
        link = 'https://test.harpiya.com/sms/3/'
        self.env['link.tracker'].search([('url', '=', link)]).unlink()
        new_body = self.env['link.tracker']._convert_links_text('Welcome to %s !' % link, self.tracker_values)
        self.assertIn(link, new_body)
        self.assertFalse(self.env['link.tracker'].search([('url', '=', link)]))
    ...
    def test_sms_body_link_shorten_suffix(self):
        ...
        sms_0 = self.env['sms.sms'].create({
            'body': 'Welcome to https://test.harpiya.com',
            'number': '12',
            'mailing_id': mailing.id,
        })
        sms_1 = self.env['sms.sms'].create({
            'body': 'Welcome to https://test.harpiya.com/r/RAOUL',
            'number': '12',
        })
        ...
        sms_3 = self.env['sms.sms'].create({
            'body': 'Welcome to https://test.harpiya.com/leodagan/r/RAOUL',
            'number': '12', 'mailing_id': mailing.id,
        })

        res = (sms_0 | sms_1 | sms_2 | sms_3)._update_body_short_links()
        self.assertEqual(res[sms_0.id], 'Welcome to https://test.harpiya.com')
        self.assertEqual(res[sms_1.id], 'Welcome to https://test.harpiya.com/r/RAOUL')
        self.assertEqual(res[sms_2.id], 'Welcome to https://test.harpiya.com/r/RAOUL/s/%s' % sms_2.id)
        self.assertEqual(res[sms_3.id], 'Welcome to https://test.harpiya.com/leodagan/r/RAOUL')
