...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        if someone pinged harpiyabot.
         ...
         :param record: the mail_thread (or mail_channel) where the user
            message was posted/harpiyabot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == harpiyabot_id:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=harpiyabot_id, message_type=message_type, subtype_id=subtype_id)
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            ...
            if harpiyabot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                ...
                return _("Wow you are a natural!<br/>Ping someone to grab its attention with @nameoftheuser. <b>Try to ping me using @HarpiyaBot</b> in a sentence.")
            elif harpiyabot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.harpiyabot_state = "idle"
                return _("Yep, I am here! 🎉 <br/>You finished the tour, you can <b>close this chat window</b>. Enjoy discovering Harpiya.")
            elif harpiyabot_state == "idle" and (_('start the tour') in body.lower()):
            ...
            # easter eggs
            ...
            elif harpiyabot_state == "idle" and (('help' in body) or _('help') in body):
                return _("I'm just a bot... :( You can check <a href=\"https://www.harpiya.com/page/docs\">our documentation</a>) for more information!")
                ...
                if harpiyabot_state == 'onboarding_emoji':
                    ...
                    return _("Sorry, I am not listening. To get someone's attention, <b>ping him</b>. Write \"@harpiyabot\" and select me.")
            ...
            return random.choice([_("Yep, HarpiyaBot is in the place!"), _("Pong.")])
        return False
    ...
    def _is_bot_pinged(self, values):
        harpiyabot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        return harpiyabot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        harpiyabot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return harpiyabot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
