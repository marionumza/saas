...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a Harpiya DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of harpiya DB
        2 - token. This token will be compared to the token of Harpiya. He have 1 hour lifetime
        """
...
class ConnectionManager(Thread):
    ...
    def _connect_box(self):
        ...
        req = requests.post('https://iot-proxy.harpiya.com/harpiya-enterprise/iot/connect-box', json=data, verify=False)
...
class Manager(Thread):
    ...
    def load_drivers(self):
        """
        This method loads local files: 'harpiya/addons/hw_drivers/drivers'
    ...
    def run(self):
        """
        Thread that will check connected/disconnected device, load drivers if needed and contact the harpiya server with the updates
        """
