...
from harpiya.addons.hw_drivers.controllers.driver import Driver, event_manager, iot_devices

try:
    from harpiya.addons.hw_drivers.controllers.driver import cm
except:
    cm = None
...
class DisplayDriver(Driver):
    ...
    def load_url(self):
        url = None
        if helpers.get_harpiya_server_url():
            # disable certifiacte verification
            urllib3.disable_warnings()
            http = urllib3.PoolManager(cert_reqs='CERT_NONE')
            try:
                response = http.request('GET', "%s/iot/box/%s/screen_url" % (helpers.get_harpiya_server_url(), helpers.get_mac_address()))
