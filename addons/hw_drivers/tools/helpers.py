...
def add_credential(db_uuid, enterprise_code):
    write_file('harpiya-db-uuid.conf', db_uuid)
    write_file('harpiya-enterprise-code.conf', enterprise_code)
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Harpiya DB and
        ...
        try:
            ...
            if response.status == 200:
                ...
                if db_branch != local_branch:
                    ...
                    subprocess.check_call(["rm", "-rf", "/home/pi/harpiya/addons/hw_drivers/drivers/*"])
...
def check_image():
    ...
    """
    url = 'http://nightly.harpiya.com/master/posbox/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def get_harpiya_server_url():
    ...
    if not ap:
        return False
    return read_file_first_line('harpiya-remote-server.conf')
...
def load_certificate():
    ...
    """
    db_uuid = read_file_first_line('harpiya-db-uuid.conf')
    enterprise_code = read_file_first_line('harpiya-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.harpiya.com/harpiya-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('harpiya-subject.conf', result['subject_cn'])
            subprocess.check_call(["sudo", "mount", "-o", "remount,rw", "/"])
...
def download_drivers(auto=True):
    ...
    if server:
        ...
        try:
            ...
            if resp.data:
                subprocess.check_call(["sudo", "mount", "-o", "remount,rw", "/"])
                drivers_path = Path.home() / 'harpiya/addons/hw_drivers/drivers'
