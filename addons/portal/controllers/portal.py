...
from harpiya import fields as harpiya_fields, http, tools, _, SUPERUSER_ID
from harpiya.exceptions import ValidationError, AccessError, MissingError, UserError
from harpiya.http import content_disposition, Controller, request, route
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': harpiya_fields.Date.to_string(harpiya_fields.Date.from_string(date_begin)),
                'date_end': harpiya_fields.Date.to_string(harpiya_fields.Date.from_string(date_end)),
                'name': label,
