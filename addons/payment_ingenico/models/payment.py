...
from harpiya.addons.payment.models.payment_acquirer import ValidationError
from harpiya.addons.payment_ingenico.controllers.main import OgoneController
from harpiya.addons.payment_ingenico.data import ogone
from harpiya.http import request
from harpiya.tools import DEFAULT_SERVER_DATE_FORMAT, ustr
from harpiya.tools.float_utils import float_compare, float_repr, float_round
...
class PaymentAcquirerOgone(models.Model):
    ...
    def _ogone_generate_shasign(self, inout, values):
        ...
        :param string inout: 'in' (harpiya contacting ogone) or 'out' (ogone
                             contacting harpiya). In this last case only some
...
class PaymentTxOgone(models.Model):
    ...
    def ogone_s2s_do_transaction(self, **kwargs):
        ...
        reference = self.reference or "HARPIYA-%s-%s" % (datetime.datetime.now().strftime('%y%m%d_%H%M%S'), self.partner_id.id)
    ...
    def _ogone_s2s_get_tx_status(self):
        account = self.acquirer_id
        #reference = tx.reference or "HARPIYA-%s-%s" % (datetime.datetime.now().strftime('%Y%m%d_%H%M%S'), tx.partner_id.id)
...
class PaymentToken(models.Model):
    ...
    def ogone_create(self, values):
        if values.get('cc_number'):
            ...
            alias = 'HARPIYA-NEW-ALIAS-%s' % time.time()
