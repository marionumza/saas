    ...
    def _get_transifex_url(self):
        ...
        for addon_path in harpiya.addons.__path__:
            ...
            if os.path.isfile(tx_path):
                ...
                # first section is [main], after [harpiya-11.sale]
                tx_sections.extend(tx_config_file.sections()[1:])

            # parent directory ad .tx/config is root directory in harpiya/harpiya
        ...
        if not base_url or not tx_sections:
            ...
            # using ini files like '[harpiya-master.website_sale]'
            ...
            for translation in self:
                ...
                # e.g. https://www.transifex.com/harpiya/harpiya-10/translate/#fr/sale/42?q=text'Sale+Order'
