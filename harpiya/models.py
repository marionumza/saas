...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (OpenERP) module name can be in the ``harpiya.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``harpiya.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['harpiya', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = harpiya.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or harpiya.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from harpiya.fields import Char, Integer
...
""")
    ...
    def write(self, vals):
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~harpiya.fields.Date` and
              :class:`~harpiya.fields.Datetime` fields use strings as values
        ...
        * .. _openerp/models/relationals/format:
          ...
          :class:`~harpiya.fields.One2many` and
          :class:`~harpiya.fields.Many2many` use a special "commands" format to
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~harpiya.api.Environment`
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/harpiya/master-nochange-fp/harpiya/addons/test_new_api/tests/test_new_fields.py
