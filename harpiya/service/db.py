...
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            harpiya.modules.db.initialize(cr)
            harpiya.tools.config['load_language'] = lang
            cr.commit()

        registry = harpiya.modules.registry.Registry.new(db_name, demo, None, update_module=True)
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with harpiya.tools.osutil.tempdir() as dump_dir:
            ...
            harpiya.tools.exec_pg_command(*cmd)
            if stream:
                harpiya.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                harpiya.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = harpiya.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with harpiya.tools.osutil.tempdir() as dump_dir:
        ...
        if harpiya.tools.exec_pg_command(pg_cmd, *pg_args):
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        harpiya.tools.config['update']['base'] = True
        harpiya.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
def list_dbs(force=False):
    ...
    if not harpiya.tools.config['dbfilter'] and harpiya.tools.config['db_name']:
        ...
        res = sorted(db.strip() for db in harpiya.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [harpiya.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if harpiya.tools.table_exists(cr, 'ir_module_module'):
...
def exp_list_lang():
    return harpiya.tools.scan_languages()
