...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, harpiya.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, harpiya.tools.ustr(e.name))
    ...
    elif isinstance(e, harpiya.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = harpiya.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, harpiya.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(harpiya.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(harpiya.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(harpiya.tools.ustr(value), write)
...
def application_unproxied(environ, start_response):
    ...
    # harpiya.service.web_services.objects_proxy.dispatch().
    ...
    with harpiya.api.Environment.manage():
        result = harpiya.http.root(environ, start_response)
