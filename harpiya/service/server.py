...
from harpiya.modules.module import run_unit_tests, get_test_modules
from harpiya.modules.registry import Registry
from harpiya.release import nt_service_name
from harpiya.tools import config
from harpiya.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    # is included in all Python 3 releases installed on all operating systems supported by Harpiya.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(harpiya, 'phoenix', False):
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from harpiya.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = harpiya.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(harpiya.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="harpiya.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="harpiya.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(harpiya, 'phoenix', None):
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(harpiya.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = harpiya.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('harpiya.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from harpiya.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(harpiya.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            harpiya.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if harpiya.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    result = harpiya.modules.module.HarpiyaTestRunner().run(suite)
        ...
        try:
            ...
            if config['test_file']:
                ...
                if not os.path.isfile(test_file):
                    ...
                    with harpiya.api.Environment.manage():
            ...
            if config['test_enable']:
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, harpiya.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    ...
    harpiya.service.wsgi_server._patch_xmlrpc_marshaller()

    if harpiya.evented:
        server = GeventServer(harpiya.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(harpiya.service.wsgi_server.application)
        ...
        server = ThreadedServer(harpiya.service.wsgi_server.application)
    ...
    if getattr(harpiya, 'phoenix', False):
        _reexec()
