...
from harpiya.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from harpiya.tools import float_compare, single_email_re
...
ADDONS_PATH = harpiya.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = harpiya.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = harpiya.SUPERUSER_ID


def get_db_name():
    db = harpiya.tools.config['db_name']
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = harpiya.registry(get_db_name())
        ...
        #: :class:`~harpiya.api.Environment` for the current test case
        self.env = api.Environment(self.cr, harpiya.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.registry = harpiya.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.env = api.Environment(cls.cr, harpiya.SUPERUSER_ID, {})
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = harpiya.http.root.session_store.new()
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "harpiya.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "harpiya.__DEBUG__.services['web_tour.tour'].tours.%s.ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~harpiya.fields.Many2one` fields assign a singleton recordset::
    ...
    Finally :class:`~harpiya.fields.One2many` are reified as
    :class:`~harpiya.tests.common.O2MProxy`.

    Because the :class:`~harpiya.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~harpiya.tests.common.O2MProxy.new` and
    :meth:`~harpiya.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: harpiya.models.Model
    ...
    :type view: int | str | harpiya.model.Model
    ...
    def _init_from_defaults(self, model):
        ...
        def cleanup(k, v):
            if fields[k]['type'] == 'one2many':
                ...
                return harpiya.fields.Datetime.to_string(v)
...
def record_to_values(fields, record):
    ...
    for f, descr in fields.items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = harpiya.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = harpiya.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return harpiya.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return harpiya.fields.Date.to_string(value)
    return value
