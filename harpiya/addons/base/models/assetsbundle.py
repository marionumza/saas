...
from harpiya import fields, tools, SUPERUSER_ID
from harpiya.tools.pycompat import to_text
from harpiya.http import request
from harpiya.modules.module import get_resource_path
from .qweb import escape
import psycopg2
from harpiya.tools import func, misc
        ...
        return """
            (function (message) {
                ...
                async function onDOMContentLoaded() {
                    var harpiya = window.top.harpiya;
                    if (!harpiya || !harpiya.define) {
                        useAlert();
                        return;
                    }
                    ...
                    await new Promise(resolve => {
                        const noLazyTimeout = setTimeout(() => resolve(), 10); // 10 since need to wait for promise resolutions of harpiya.define
                        harpiya.define('AssetsBundle.PotentialLazyLoading', function (require) {
                            'use strict';
                    ...
                    var alertTimeout = setTimeout(useAlert, 10); // 10 since need to wait for promise resolutions of harpiya.define
                    harpiya.define('AssetsBundle.ErrorMessage', function (require) {
                        'use strict';
