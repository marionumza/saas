...
class Cloc(object):
    ...
    def count_customization(self, env):
        ...
        for a in env['ir.actions.server'].browse(data.keys()):
            self.book(data[a.id] or "harpiya/studio", "ir.actions.server/%s: %s" % (a.id, a.name), self.parse_py(a.code))
        ...
        for f in env['ir.model.fields'].browse(data.keys()):
            self.book(data[f.id] or "harpiya/studio", "ir.model.fields/%s: %s" % (f.id, f.name), self.parse_py(f.compute))
    ...
    def count_database(self, database):
        with harpiya.api.Environment.manage():
            registry = harpiya.registry(config['db_name'])
