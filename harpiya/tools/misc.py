...
from harpiya.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if harpiya.tools.config['db_host']:
        env['PGHOST'] = harpiya.tools.config['db_host']
    if harpiya.tools.config['db_port']:
        env['PGPORT'] = str(harpiya.tools.config['db_port'])
    if harpiya.tools.config['db_user']:
        env['PGUSER'] = harpiya.tools.config['db_user']
    if harpiya.tools.config['db_password']:
        env['PGPASSWORD'] = harpiya.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    ...
    """
    import harpiya.modules as addons
    adps = harpiya.addons.__path__
...
def _fileopen(path, mode, basedir, pathinfo, basename=None):
    ...
    import harpiya.modules as addons
    paths = harpiya.addons.__path__ + [config['root_path']]
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = harpiya.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = harpiya.fields.Datetime.from_string(value)
            value = harpiya.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = harpiya.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = harpiya.fields.Datetime.from_string(value)
    else:
        timestamp = value
