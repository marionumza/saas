#harpiya.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "harpiya.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="harpiya.http.rpc.request:DEBUG", help='shortcut for --log-handler=harpiya.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="harpiya.http.rpc.response:DEBUG", help='shortcut for --log-handler=harpiya.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="harpiya.http:DEBUG", help='shortcut for --log-handler=harpiya.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="harpiya.sql_db:DEBUG", help='shortcut for --log-handler=harpiya.sql_db:DEBUG')
    ...
    def _parse_config(self, args=None):
        ...
        if os.name == 'nt':
            ...
            rcfilepath = os.path.expanduser('~/.harpiyarc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.harpiyarc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.harpiyarc'.")
    ...
    def _is_addons_path(self, path):
        from harpiya.modules.module import MANIFEST_NAMES
