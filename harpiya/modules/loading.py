...
import harpiya.modules.graph
import harpiya.modules.migration
import harpiya.modules.registry
import harpiya.tools as tools

from harpiya import api, SUPERUSER_ID
from harpiya.modules.module import adapt_version, initialize_sys_path, load_openerp_module
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            harpiya.registry(cr.dbname).clear_caches()
    ...
    processed_modules = []
    loaded_modules = []
    registry = harpiya.registry(cr.dbname)
    migrations = harpiya.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        if needs_update:
            ...
            overwrite = harpiya.tools.config["overwrite_existing_translations"]
            ...
            if tools.config.options['test_enable']:
                ...
                report.record_result(harpiya.modules.module.run_unit_tests(module_name))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, harpiya.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        if not harpiya.modules.db.is_initialized(cr):
        ...
        # harpiya.modules.registry.Registry.new().
        registry = harpiya.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                registry = harpiya.modules.registry.Registry.new(
