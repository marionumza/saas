...
from harpiya.sql_db import TestCursor
from harpiya.tools import (assertion_report, config, existing_tables, ignore,
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # harpiya.service.wsgi_server.application
        ...
        with cls._lock:
            with harpiya.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        harpiya.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
                        harpiya.modules.reset_modules_state(db_name)
                        raise
                except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = harpiya.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = harpiya.modules.db.has_unaccent(cr)
            if harpiya.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = harpiya.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = harpiya.api.Environment(cr, SUPERUSER_ID, context)
