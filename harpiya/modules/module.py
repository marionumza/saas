...
from harpiya import SUPERUSER_ID, api
...
# ad_paths is a deprecated alias, please use harpiya.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"harpiya.modules.module.ad_paths" is a deprecated proxy to '
        '"harpiya.addons.__path__".', DeprecationWarning, stacklevel=2)
    return harpiya.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "harpiya.addons".',
    ...
    def load_module(self, name):
        ...
        harpiya_name = re.sub(r'^openerp.addons.(\w+)$', r'harpiya.addons.\g<1>', name)

        harpiya_module = sys.modules.get(harpiya_name)
        if not harpiya_module:
            harpiya_module = importlib.import_module(harpiya_name)

        sys.modules[name] = harpiya_module

        return harpiya_module
...
class HarpiyaHook(object):
    """ Makes harpiya package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to harpiya.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'harpiya\g<1>', name)
...
def get_module_filetree(module, dir='.'):
    ...
    files = harpiya.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in harpiya.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('harpiya.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['harpiya.addons.' + module_name], info['post_load'])()
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('harpiya.addons', module)

    try:
        importlib.import_module('harpiya.upgrade.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('harpiya.upgrade', module)
...
class HarpiyaTestResult(unittest.result.TestResult):
    """
    ...
    def getErrorCallerInfo(self, error, test):
        ...
        # only test case should be executed in harpiya, this is only a safe guard
...
def run_unit_tests(module_name, position='at_install'):
    ...
    """
    global current_test
    from harpiya.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            t0 = time.time()
            t0_sql = harpiya.sql_db.sql_counter
            _logger.info('%s running tests.', m.__name__)
            result = HarpiyaTestRunner().run(suite)
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, harpiya.sql_db.sql_counter - t0_sql)
