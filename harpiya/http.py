...
# *broken*
NO_POSTMORTEM = (harpiya.exceptions.except_orm,
                 harpiya.exceptions.AccessDenied,
                 harpiya.exceptions.Warning,
                 harpiya.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                harpiya.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = harpiya.service.common.dispatch
        elif service_name == 'db':
            dispatch = harpiya.service.db.dispatch
        elif service_name == 'object':
            dispatch = harpiya.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                harpiya.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                harpiya.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except harpiya.exceptions.DeferredException as e:
        _logger.exception(harpiya.tools.exception_to_unicode(e))
        harpiya.tools.debugger.post_mortem(harpiya.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(harpiya.tools.exception_to_unicode(e))
        harpiya.tools.debugger.post_mortem(harpiya.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all Harpiya Web request types, mostly deals with
    ...
    @property
    def cr(self):
        """ :class:`~harpiya.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~harpiya.api.Environment` bound to current request. """
        if self._env is None:
            self._env = harpiya.api.Environment(self.cr, self.uid, self.context)
        return self._env

    @lazy_property
    def session(self):
        """ :class:`HarpiyaSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            harpiya.tools.debugger.post_mortem(
                harpiya.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return harpiya.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Harpiya implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <harpiya.http.WebRequest.csrf_token`, the
          :data:`~harpiya.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          Harpiya) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (harpiya.exceptions.Warning, harpiya.exceptions.except_orm,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "Harpiya Session Invalid"
...
def serialize_exception(e):
    ...
    if isinstance(e, harpiya.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, harpiya.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, harpiya.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    ...
    elif isinstance(e, harpiya.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, harpiya.exceptions.except_orm):
...
HTTP methods). See
https://www.harpiya.com/documentation/13.0/reference/http.html#csrf for
more details.
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["harpiya", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('harpiya.addons.') and c.__module__.split(".")[2] in modules
...
class HarpiyaSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(HarpiyaSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = harpiya.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = harpiya.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or harpiya.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        ...
        return werkzeug.contrib.sessions.FilesystemSessionStore(
            path, session_class=HarpiyaSession, renew_missing=True)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        routing_map = werkzeug.routing.Map(strict_slashes=False, converters=None)
        for url, endpoint, routing in harpiya.http._generate_routing_rules([''] + harpiya.conf.server_wide_modules, True):
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        harpiya.registry(db).check_signaling()
                        with harpiya.tools.mute_logger('harpiya.sql_db'):
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if harpiya.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = harpiya.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif harpiya.tools.config['db_name']:
        ...
        exposed_dbs = set(db.strip() for db in harpiya.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = harpiya.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = harpiya.tools.ustr(filename)
