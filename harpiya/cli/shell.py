...
class Shell(Command):
    ...
    def init(self, args):
        ...
        harpiya.cli.server.report_configuration()
        harpiya.service.server.start(preload=[], stop=True)
        ...
        with harpiya.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = harpiya.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = harpiya.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
