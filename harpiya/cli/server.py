...
__author__ = harpiya.release.author
__version__ = harpiya.release.version
...
def report_configuration():
    ...
    """
    config = harpiya.tools.config
    _logger.info("Harpiya version %s", __version__)
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = harpiya.modules.registry.Registry.new(dbname)
        with harpiya.api.Environment.manage():
            with registry.cursor() as cr:
                harpiya.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with harpiya.api.Environment.manage():
        with registry.cursor() as cr:
            harpiya.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    harpiya.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                harpiya.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except harpiya.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        harpiya.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = harpiya.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)
